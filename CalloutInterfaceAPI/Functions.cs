namespace CalloutInterfaceAPI
{
    ...
    public static class Functions
    {
        ...
        /// <summary>
        /// Gets the <see cref="Callout"/> for an LSPDFR callout handle (Callout Interface’s resolver). Returns null if Callout Interface is unavailable, the handle is null, or resolution fails.
        /// </summary>
        /// <param name="handle">The callout handle from <c>OnCalloutDisplayed</c> / related LSPDFR events.</param>
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static Callout GetCalloutFromHandle(LHandle handle)
        {
            if (!IsCalloutInterfaceAvailable || handle == null)
            {
                return null;
            }
            try
            {
                return External.CalloutInterfaceInvoker.GetCalloutFromHandle(handle);
            }
            catch
            {
                return null;
        ...
        /// <summary>
        /// Updates the in-game Callout Interface CAD / unit status line when the installed CI exposes a compatible API (otherwise no-op). Schedules on a game <see cref="Rage.GameFiber"/>; safe from HTTP or worker threads.
        /// </summary>
        /// <param name="statusText">Free-text status (e.g. 10-8, 10-97).</param>
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static void PublishCadUnitStatus(string statusText)
        {
            if (string.IsNullOrWhiteSpace(statusText) || !IsCalloutInterfaceAvailable)
            {
                return;
            }
            string line = statusText.Trim();
            Rage.GameFiber.StartNew(() =>
            {
                try
                {
                    External.CalloutInterfaceCadStatusInvoker.TrySetStatusLine(line);
                }
                catch (Exception ex)
                {
                    Rage.Game.LogTrivial($"CalloutInterfaceAPI: PublishCadUnitStatus failed: {ex.Message}");
                }
            });
        }
