(async function () {
  ...
  const render = async () => {
    ...
    const filteredCases = courtCases
      ...
      .filter((c) => {
        if (state.status !== "all" && `${c.Status}` !== state.status)
          return false;
    ...
    for (const courtCase of filteredCases) {
      listContainer.appendChild(
        await createCourtCaseElement(courtCase, language, refreshCourtList),
      );
    }
...
function createControls(language) {
  ...
  searchInput.setAttribute(
    "aria-label",
    language.court.searchPlaceholder || "Search case #, name, report",
  );
  ...
  const statusMap = language.court.statusMap || [
    "Pending",
    "Convicted",
    "Acquitted",
    "Dismissed",
...
async function createCourtCaseElement(courtCase, language, refreshCourtList) {
  ...
  const courtDateLabel =
    courtCase.Status === 0
      ? language.court.trialDate || "Trial"
      : language.court.courtDate || "Court";
  const dateForRow =
    courtCase.Status === 0 && courtCase.ResolveAtUtc
      ? courtCase.ResolveAtUtc
      : courtCase.HearingDateUtc ||
        courtCase.ResolveAtUtc ||
        courtCase.LastUpdatedUtc ||
        "";
  ...
  const synthBadgeTitle =
    (language.court && language.court.syntheticCaseBadge) ||
    "Prior disposition (reconstructed record)";
  ...
  chargesSearchResponseWrapper.classList.add(
  ...
  chargesSearchResponseTitle.innerHTML = escapeHtml(
    language.court?.charges ?? "Charges",
  );
  ...
  for (const charge of courtCase.Charges || []) {
    ...
    const convicted =
      isResolved &&
      (charge.Outcome === 1 ||
        (charge.Outcome === 0 && courtCase.Status === 1));
    if (!isResolved || convicted) {
      ...
      const daysForTime =
        isResolved && convicted && charge.SentenceDaysServed != null
    ...
    if (!isResolved) {
      ...
      const mxd =
        charge.MaxDays != null && typeof charge.MaxDays === "number"
          ? charge.MaxDays
          : null;
      ...
      detailValue = await getChargeDetailsString(
        charge.Fine || 0,
        charge.Time,
        { hideIncarceration: true },
      );
    }
    if (isResolved) {
      const outcomeMap = {
      ...
      const displayOutcome =
        charge.Outcome === 0 || charge.Outcome === undefined
          ? courtCase.Status
          : charge.Outcome;
      const outcomeLabel =
        outcomeMap[displayOutcome] ||
        language.court.chargeOutcomePending ||
        "Pending";
  ...
  if (timelineDates.length > 0) {
    const timelineValues = await Promise.all(
      timelineDates.map(([, v]) => formatIsoDate(v)),
    );
    const timelineSection = document.createElement("div");
    timelineSection.classList.add("courtTimelineSection");
    timelineSection.appendChild(
      createSectionHeader(language.court.caseTimeline || "Case Timeline"),
    );
  ...
  if (courtCase.Status === 0 || hasAttachedReports) {
    ...
    attachedSection.appendChild(
      createSectionHeader(
        language.court.attachedReports || "Attached reports (evidence)",
      ),
    );
    if (courtCase.Status === 0) {
      const attachedHelp = document.createElement("p");
      attachedHelp.className = "courtAttachedReportsHelp";
      attachedHelp.textContent =
        language.court.attachedReportsHelp ||
        "Court scores every attachment. Strong ties to the defendant and charges add the most; loose ties still register lower.";
    ...
    const reportTypeLabels = {
      ...
      trafficIncident:
        language.court.reportTypeTrafficIncident || "Traffic Incident",
      injury: language.court.reportTypeInjury || "Injury",
      propertyEvidence:
        language.court.reportTypePropertyEvidence || "Property & Evidence",
    };

    attachedIds.forEach((reportId) => {
      const sum = reportSummaries.find((s) => s && s.id === reportId);
      const typeLabel = sum?.type
        ? reportTypeLabels[sum.type] || sum.typeLabel || "—"
        : "—";
      const row = document.createElement("div");
      row.classList.add("courtAttachedReportIdRow");
      const span = document.createElement("span");
      span.textContent =
        courtCase.Status !== 0 ? `${reportId} (${typeLabel})` : reportId;
      ...
      if (courtCase.Status === 0) {
        ...
        detachBtn.addEventListener("click", async function () {
          ...
          const res = await (
            await fetch("/post/detachReportFromCourtCase", {
              ...
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                courtCaseNumber: courtCase.Number,
                reportId,
              }),
            })
          ).text();
          ...
          if (res === "OK") await refreshCourtList();
            ...
            topWindow.showNotification(
              language.court.saveCaseError || "Error",
              "error",
            );
        });
        ...
        linkSpan.addEventListener("click", () =>
          openIdInReport(reportId, reportType),
        );
    ...
    const now = new Date();
    const resolveAt = courtCase.ResolveAtUtc
      ? new Date(courtCase.ResolveAtUtc)
      : null;
    ...
    if (canAttach) {
      ...
      lockedHint.textContent =
        language.court.attachReportLockedHint ||
        "Evidence links are locked after the scheduled court date.";
  ...
  if (hasConvictedDisposition) {
    ...
    totalFineWrapper.appendChild(
      createReadOnlyInput(await getCurrencyString(totalFine)),
    );
    inputWrapper.appendChild(totalFineWrapper);

    const totalTimeWrapper = document.createElement("div");
    totalTimeWrapper.appendChild(
      createLabel(language.court.totalIncarceration),
    );
    totalTimeWrapper.appendChild(
      createReadOnlyInput(await getTotalTimeString(totalTime, lifeSentences)),
    );
    if (totalTime > 0 || lifeSentences > 0)
      inputWrapper.appendChild(totalTimeWrapper);
  }

  const riskWrapper = document.createElement("div");
  inputWrapper.appendChild(
    createSectionHeader(
      language.court.sectionScoring || "Scoring & Sentencing",
    ),
  );
  riskWrapper.appendChild(
    createLabel(language.court.repeatOffenderScore || "Repeat Offender Score"),
  );
  riskWrapper.appendChild(
    createReadOnlyInput(`${courtCase.RepeatOffenderScore || 0}`),
  );
  inputWrapper.appendChild(riskWrapper);

  const multiplierWrapper = document.createElement("div");
  multiplierWrapper.appendChild(
    createLabel(language.court.sentenceMultiplier || "Sentence Multiplier"),
  );
  multiplierWrapper.appendChild(
  ...
  courtDistrictWrapper.appendChild(
    createReadOnlyInput(courtCase.CourtDistrict || "-"),
  );
  ...
  courtNameWrapper.appendChild(
    createLabel(language.court.courtName || "Court Name"),
  );
  courtNameWrapper.appendChild(
    createReadOnlyInput(
      courtCase.CourtName
  ...
  prosecutorWrapper.appendChild(
    createLabel(language.court.prosecutor || "Prosecutor"),
  );
  prosecutorWrapper.appendChild(
    createReadOnlyInput(courtCase.ProsecutorName || "-"),
  );
  inputWrapper.appendChild(prosecutorWrapper);

  const defenseAttorneyWrapper = document.createElement("div");
  defenseAttorneyWrapper.appendChild(
    createLabel(language.court.defenseAttorney || "Defense Attorney"),
  );
  defenseAttorneyWrapper.appendChild(
    createReadOnlyInput(courtCase.DefenseAttorneyName || "-"),
  );
  inputWrapper.appendChild(defenseAttorneyWrapper);

  const severityWrapper = document.createElement("div");
  severityWrapper.appendChild(
    createLabel(language.court.severityScore || "Severity Score"),
  );
  severityWrapper.appendChild(
    createReadOnlyInput(`${courtCase.SeverityScore || 0}`),
  );
  inputWrapper.appendChild(severityWrapper);

  const evidenceScore = Math.max(0, Number(courtCase.EvidenceScore) || 0);
  const evidenceBand =
    evidenceScore < 35 ? "Low" : evidenceScore < 60 ? "Medium" : "Strong";
  const evidenceBandLabel =
    evidenceBand === "Low"
      ? language.court.evidenceBandLow || "Low"
      : evidenceBand === "Medium"
        ? language.court.evidenceBandMedium || "Medium"
        : language.court.evidenceBandStrong || "Strong";
  const evidenceWrapper = document.createElement("div");
  evidenceWrapper.appendChild(
    createLabel(language.court.evidenceScore || "Evidence Score"),
  );
  evidenceWrapper.appendChild(
    createReadOnlyInput(`${evidenceScore} (${evidenceBandLabel})`),
  );
  ...
  if (evidenceBand === "Low") {
    ...
    evidenceBandNote.textContent =
      language.court.evidenceBandLowNote ||
      "Limited physical evidence – case may rely on officer testimony.";
  ...
  evidenceToggleBtn.innerText =
    language.court.viewEvidenceBtn || "View Evidence Breakdown";
  evidenceToggleBtn.title =
    language.court.viewEvidenceBtn || "View prosecution exhibits";
  ...
  const hasAnyRealEvidence =
    (courtCase.EvidenceHadWeapon ?? false) ||
    (courtCase.EvidenceWasWanted ?? false) ||
    (courtCase.EvidenceAssaultedPed ?? false) ||
    (courtCase.EvidenceDamagedVehicle ?? false) ||
    (courtCase.EvidenceResisted ?? false) ||
    (courtCase.EvidenceHadDrugs ?? false) ||
    (courtCase.EvidenceUseOfForce ?? false) ||
    (courtCase.EvidenceWasDrunk ?? false) ||
  ...
  noEvidenceNote.innerText = hasAnyRealEvidence
    ? language.court.evidenceCapturedNote ||
      "In-game evidence was captured for this case."
    : language.court.evidenceNotCapturedNote ||
      "No in-game evidence captured. Either the arrest was processed before evidence hooks fired, or in-game integration (LSPDFR / Policing Redefined / StopThePed) did not supply hooks for this ped.";
  ...
  repeatLbl.innerText =
    language.court.repeatOffenderScore || "Repeat Offender Score";
  ...
  multiplierLbl.innerText =
    language.court.sentenceMultiplier || "Sentence Multiplier";
  ...
  const exhibitLabels = [
    {
      key: "EvidenceHadWeapon",
      label:
        (language.court.exhibitFirearm || "Firearm recovered") +
        (courtCase.EvidenceFirearmTypesBreakdown?.length > 0
          ? ` (${courtCase.EvidenceFirearmTypesBreakdown.join(", ")})`
          : ""),
    },
    {
      key: "EvidenceHadDrugs",
      label:
        (language.court.exhibitDrugs || "Drugs") +
        (courtCase.EvidenceDrugTypesBreakdown?.length > 0
          ? ` (${courtCase.EvidenceDrugTypesBreakdown.join(", ")})`
          : ""),
    },
    {
      key: "_arrestReport",
      label: language.court.exhibitArrestReport || "Arrest report",
      always: true,
    },
    {
      key: "EvidenceWasWanted",
      label: language.court.exhibitWarrant || "Active warrant documentation",
    },
    {
      key: "EvidenceAssaultedPed",
      label: language.court.exhibitAssault || "Assault evidence",
    },
    {
      key: "EvidenceDamagedVehicle",
      label:
        language.court.exhibitVehicleDamage ||
        "Vehicle/property damage evidence",
    },
    {
      key: "EvidenceResisted",
      label: language.court.exhibitResistance || "Resistance evidence",
    },
    {
      key: "EvidenceUseOfForce",
      label: language.court.exhibitUseOfForce || "Use of force documentation",
    },
    {
      key: "EvidenceWasDrunk",
      label: language.court.exhibitIntoxication || "Intoxication evidence",
    },
    {
      key: "EvidenceWasFleeing",
      label: language.court.exhibitFleeing || "Fleeing evidence",
    },
    {
      key: "EvidenceViolatedSupervision",
      label: language.court.exhibitSupervision || "Supervision violation",
    },
    {
      key: "EvidenceWasPatDown",
      label: language.court.exhibitPatDown || "Pat-down/search evidence",
    },
    {
  ...
  for (const ex of exhibitLabels) {
    ...
    if (ex.always || courtCase[ex.key])
      exhibitRow.classList.add("evidenceBreakdownRowActive");
    const lbl = document.createElement("span");
    lbl.innerText = `Exhibit ${letter}: ${ex.label}`;
    const val = document.createElement("span");
    val.classList.add("evidenceBreakdownValue");
    val.innerText = ex.always
      ? courtCase.ReportId
        ? courtCase.ReportId
        : "—"
  ...
  const drugAssessments = Array.isArray(courtCase.DrugEvidenceAssessments)
    ? courtCase.DrugEvidenceAssessments
    : [];
  for (const assessment of drugAssessments) {
    if (!assessment || !assessment.TypeMatched) continue;
    const row = document.createElement("div");
    row.classList.add("evidenceBreakdownRow", "evidenceExhibitRow");
    if (assessment.SupportsCharge)
      row.classList.add("evidenceBreakdownRowActive");
    const lbl = document.createElement("span");
    const qty = assessment.Quantity ? ` (${assessment.Quantity})` : "";
    lbl.innerText = `${assessment.ChargeName || "Drug charge"}: ${assessment.DrugType || "drug"}${qty} — ${assessment.Reason || "quantity reviewed"}`;
    const val = document.createElement("span");
    val.classList.add("evidenceBreakdownValue");
    val.innerText = assessment.SupportsCharge
      ? language.court.evidenceYes || "YES"
      : assessment.SupportedLesserLevel || "LIMITED";
    row.appendChild(lbl);
    row.appendChild(val);
  ...
  // Charges filed
  const charges = (courtCase.Charges || []).filter(
  ...
  charges.forEach((charge) => {
    ...
    chargeVal.innerText = charge.IsArrestable
      ? language.court.arrestable || "ARRESTABLE"
      : language.court.nonArrestable || "CIVIL";
  ...
  evidenceToggleBtn.addEventListener("click", () => {
    ...
    evidenceToggleBtn.setAttribute(
      "aria-expanded",
  ...
  prosecutionWrapper.appendChild(
    createLabelWithTooltip(
      language.court.prosecutionStrength || "Prosecution Strength",
      language.court.prosecutionStrengthTooltip ||
        "Prosecution's estimated strength based on evidence and case factors.",
    ),
  );
  prosecutionWrapper.appendChild(
  ...
  defenseWrapper.appendChild(
    createLabelWithTooltip(
      language.court.defenseStrength || "Defense Strength",
      language.court.defenseStrengthTooltip ||
        "Defense's estimated strength based on representation and case factors.",
    ),
  );
  defenseWrapper.appendChild(
  ...
  convictionChanceWrapper.appendChild(
    createLabel(language.court.convictionChance || "Conviction Probability"),
  );
  convictionChanceWrapper.appendChild(
    createReadOnlyInput(`${courtCase.ConvictionChance || 0}%`),
  );
  ...
  docketWrapper.appendChild(
    createLabelWithTooltip(
      language.court.docketPressure || "Docket Pressure",
      language.court.docketPressureTooltip,
    ),
  );
  docketWrapper.appendChild(
    createReadOnlyInput(
      `${((courtCase.DocketPressure || 0) * 100).toFixed(0)}%`,
    ),
  );
  ...
  policyWrapper.appendChild(
    createLabelWithTooltip(
      language.court.policyAdjustment || "District Policy Adjustment",
      language.court.policyAdjustmentTooltip,
    ),
  );
  policyWrapper.appendChild(
    createReadOnlyInput(
      `${((courtCase.PolicyAdjustment || 0) * 100).toFixed(1)}%`,
    ),
  );
  ...
  pleaWrapper.appendChild(
    createLabelWithTooltip(
      language.court.plea || "Plea",
      pleaTooltipText || null,
    ),
  );
  const pleaSelect = document.createElement("select");
  (language.court.pleaMap || ["Not Guilty", "Guilty", "No Contest"]).forEach(
    (pleaOption) => {
      const option = document.createElement("option");
      option.value = pleaOption;
      option.textContent = pleaOption;
      if ((courtCase.Plea || "").toLowerCase() === pleaOption.toLowerCase())
  ...
  const outcomeNotesText =
    typeof courtCase.OutcomeNotes === "string" ? courtCase.OutcomeNotes : "";
  const shouldShowOutcomeNotes =
    courtCase.Status === 0 || outcomeNotesText.trim().length > 0;
  let notesInput = null;
  if (shouldShowOutcomeNotes) {
    const notesWrapper = document.createElement("div");
    notesWrapper.classList.add("courtNotes");
    notesWrapper.appendChild(
      createLabel(language.court.outcomeNotes || "Outcome Notes"),
    );
  ...
  if (courtCase.Status !== 0) {
    ...
    reasoningWrapper.appendChild(
      createLabel(
        language.court.outcomeReasoning || "Verdict & Outcome Reasoning",
      ),
    );
    const reasoningInput = document.createElement("textarea");
    reasoningInput.value =
      typeof courtCase.OutcomeReasoning === "string"
        ? courtCase.OutcomeReasoning
        : "";
    ...
    if (courtCase.Status === 1 && courtCase.SentenceReasoning) {
      ...
      sentencingInput.value =
        typeof courtCase.SentenceReasoning === "string"
          ? courtCase.SentenceReasoning
          : "";
    ...
    if (courtCase.Status === 1) {
      await appendSentenceBreakdown(dispositionSection, courtCase, language);
    }
  ...
  if (
    courtCase.Status === 1 &&
    courtCase.LicenseRevocations &&
    courtCase.LicenseRevocations.length > 0
  ) {
    ...
    revocationsWrapper.appendChild(
      createLabel(
        language.court.licenseRevocations || "License Revocations Ordered",
      ),
    );
  ...
  const currentStatusLabel = createReadOnlyInput(
    statusMap[courtCase.Status] || "Unknown",
  );
  ...
  if (courtCase.Status === 0) {
    ...
    saveCaseBtn.addEventListener("click", async function () {
      ...
      if (response === "OK") {
        ...
        topWindow.showNotification(
          language.court.saveCaseError || "Failed to save case.",
          "error",
        );
      }
    ...
    forceResolveBtn.addEventListener("click", async function () {
      ...
      try {
        ...
        if (response.ok && payload?.success !== false) {
          topWindow.showNotification(
            language.court.forceResolveSuccess || "Case resolved.",
            "checkMark",
          );
          if (typeof refreshCourtList === "function") await refreshCourtList();
        } else {
          topWindow.showNotification(
            payload?.error ||
              language.court.forceResolveError ||
        ...
        topWindow.showNotification(
          language.court.forceResolveError || "Could not resolve case.",
          "error",
        );
      } finally {
...
function showCourtProcessingModal(language) {
  const stages =
    Array.isArray(language.court.forceResolveStages) &&
    language.court.forceResolveStages.length > 0
  ...
  title.textContent =
    language.court.forceResolveProcessing || "Processing case...";
...
function toggleCourtCaseExpanded(listItem) {
  if (
    !listItem ||
    !listItem.classList ||
    !listItem.classList.contains("courtCaseListItem")
  )
    return;
...
async function appendSentenceBreakdown(parent, courtCase, language) {
  const summary = courtCase.SentenceSummary || courtCase.sentenceSummary;
  const order = courtCase.SupervisionOrder || courtCase.supervisionOrder;
  ...
  if (summary) {
    const gross = Number(summary.GrossSentenceDays ?? summary.grossSentenceDays ?? 0);
    const netRaw = summary.NetSentenceDays ?? summary.netSentenceDays;
    const net = netRaw === null || netRaw === undefined ? null : Number(netRaw);
    const credits = Number(summary.CustodyCreditDays ?? summary.custodyCreditDays ?? 0);
    addRow(language.court.grossCustody || "Gross Custody", summary.HasLifeSentence ? await getTotalTimeString(gross, 1) : await getTotalTimeString(gross, 0));
    addRow(language.court.jailCredits || "Jail Credits", await getTotalTimeString(credits, 0));
    addRow(language.court.netCustody || "Net Custody", summary.HasLifeSentence ? language.units.life : await getTotalTimeString(Math.max(0, net || 0), 0));
    if (summary.CustodyReleaseUtc || summary.custodyReleaseUtc)
      addRow(language.court.custodyRelease || "Custody Release", await formatIsoDate(summary.CustodyReleaseUtc || summary.custodyReleaseUtc));
    if (summary.ParoleEligibilityUtc || summary.paroleEligibilityUtc)
      addRow(language.court.paroleEligibility || "Parole Eligibility", await formatIsoDate(summary.ParoleEligibilityUtc || summary.paroleEligibilityUtc));
    const fineAfter = summary.TotalFineAfterCredits ?? summary.totalFineAfterCredits;
    if (fineAfter !== undefined && fineAfter !== null)
      addRow(language.court.fineAfterCredits || "Fine After Credits", await getCurrencyString(Number(fineAfter) || 0));
  }

  if (order) {
    const conditions = Array.isArray(order.Conditions)
      ? order.Conditions
      : Array.isArray(order.conditions)
        ? order.conditions
        : [];
    addRow(language.court.supervision || "Supervision", `${order.Type || order.type || "-"} - ${order.Status || order.status || "-"}`);
...
async function getChargeDetailsString(fine, time, options = {}) {
  ...
  if (
    ...
    (minDays > 0 || (maxDays != null && maxDays > 0))
  ) {
    ...
    if (minDays === maxV) {
      ...
      const maxStr =
        maxDays == null ? language.units.life : await convertDaysToYMD(maxV);
  ...
  return incarcerationPart
    ? `${fineString} | ${incarcerationPart}`
    : fineString;
}
