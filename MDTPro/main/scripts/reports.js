(async function () {
  ...
  if (config.updateDomWithLanguageOnLoad)
    await updateDomWithLanguage("reports");
})();
...
function serializeDraft() {
  ...
  const type = document.querySelector(".createPage .typeSelector .selected")
    ?.dataset.type;
  if (!type) return;
...
async function applyReportPrefill(type) {
  try {
    ...
    if (
      prefill.reportType !== type ||
      (prefill.expires && Date.now() > prefill.expires)
    )
      return;
    ...
    const vehicleInput = el.querySelector(
      "#offenderSectionVehicleLicensePlateInput",
    );
    ...
    if (d.vehicleData) {
      ...
      if (impoundModel)
        impoundModel.value = v.ModelDisplayName || v.ModelName || "";
      if (impoundOwner) impoundOwner.value = v.Owner || "";
      if (impoundVin)
        impoundVin.value = v.VehicleIdentificationNumber || v.VinStatus || "";
    }
    ...
    if (d.pedName && type === "propertyEvidence") {
      ...
      if (
        subjectsList &&
        !Array.from(
          subjectsList.querySelectorAll(".propertyEvidenceSubjectItem"),
        ).some((item) => (item.dataset.pedName || "") === d.pedName)
      ) {
    ...
    if (sessionStorage.getItem("mdtproAttachPropertyEvidenceToArrestId")) {
      msg =
        lang.reports?.notifications?.prefilledFromArrest ??
        "Prefilled from arrest. Report will be attached after save.";
    } else {
      msg =
        d.source === "vehicleSearch"
          ? lang.reports?.notifications?.prefilledFromVehicleSearch ||
            "Prefilled from Vehicle Search"
          : lang.reports?.notifications?.prefilledFromPersonSearch ||
            "Prefilled from Person Search";
    }
...
async function onCreateButtonClick() {
  ...
  let type = document.querySelector(".listPage .typeSelector .selected")
    ?.dataset?.type;
  ...
  try {
    ...
    if (prefillRaw) {
      ...
      if (
        prefill.reportType &&
        ["impound", "injury", "trafficIncident", "propertyEvidence"].includes(
          prefill.reportType,
        )
      ) {
  ...
  if (draft && draft.type === type) {
    ...
    topWindow.showNotification(
      language.reports.notifications?.draftRestored || "Draft report restored",
      "info",
    );
  }
...
async function onListPageTypeSelectorButtonClick(type) {
  ...
  const statusLabel = (i) =>
    (language.reports?.statusMap?.[i] ?? defaultStatusLabels[i]) ||
    defaultStatusLabels[i];
  ...
  dateFromLabel.innerHTML = language.reports.list.filter?.dateFrom || "From";
  ...
  dateToLabel.innerHTML = language.reports.list.filter?.dateTo || "To";
  ...
  sortNewest.innerHTML = language.reports.list.filter?.newest || "Newest";
  sortNewest.classList.add("selected");
  sortNewest.dataset.sort = "newest";

  const sortOldest = document.createElement("button");
  sortOldest.innerHTML = language.reports.list.filter?.oldest || "Oldest";
  ...
  async function applyFilter() {
    ...
    for (const report of reports) {
      ...
      const locationStr = loc
        ? `${loc.Postal || ""} ${loc.Street || ""} ${loc.Area || ""}`.toLowerCase()
        : "";
      if (
      ...
      if (report.DriverNames) {
        for (const n of report.DriverNames) {
          if (n?.toLowerCase().includes(searchText)) {
            addToNewReports(report);
            break;
          }
        }
      ...
      if (report.VehiclePlates) {
        for (const p of report.VehiclePlates) {
          if (p?.toLowerCase().includes(searchText)) {
            addToNewReports(report);
            break;
          }
        }
      ...
      const selectedStatusBtn =
        statusButtonWrapper.querySelector("button.selected");
...
async function renderReports(reports, type) {
  ...
  for (const report of reports) {
    ...
    copyBtn.textContent =
      language.reports?.sections?.generalInformation?.copyReportId || "Copy";
    copyBtn.title =
      language.reports?.sections?.generalInformation?.copyReportId ||
    ...
    copyBtn.addEventListener("click", async (e) => {
      ...
      if (
        ...
        if (ok) {
          topWindow.showNotification(
            language.reports?.sections?.generalInformation?.copiedToClipboard ||
              "Report ID copied to clipboard.",
            "checkMark",
          );
        } else {
          topWindow.showNotification(
            language.reports?.sections?.generalInformation?.copyFailed ||
              "Could not copy.",
            "warning",
          );
        }
    ...
    const loc = report.Location;
    const locationStr = loc
      ? `${loc.Postal || ""} ${loc.Street || ""}, ${loc.Area || ""}`.trim() ||
        "—"
      : "—";
    ...
    switch (type) {
      case "incident":
        ...
        const involvedParties = [
          ...(Array.isArray(report.OffenderPedsNames)
            ? report.OffenderPedsNames
            : []),
          ...(Array.isArray(report.WitnessPedsNames)
      ...
      case "propertyEvidence": {
        const subjectNames =
          report.SubjectPedNames ||
          (report.SubjectPedName ? [report.SubjectPedName] : []);
        ...
        const drugsStr = (report.SeizedDrugs || [])
          .map((d) =>
            d && d.DrugType
              ? d.Quantity
                ? `${d.DrugType} (${d.Quantity})`
                : d.DrugType
              : "",
          )
          .filter(Boolean)
    ...
    const statusColor =
      statusColorMap[report.Status] ?? statusColorMap[3] ?? "warning";
    ...
    statusElement.innerHTML =
      (language.reports?.statusMap?.[report.Status] ??
        defaultStatusLabels[report.Status]) ||
      "Unknown";
    ...
    editButton.addEventListener("click", async function () {
      ...
      const language = await getLanguage();
      for (const iframe of topDoc.querySelectorAll(
        ".overlay .windows .window iframe",
      )) {
      ...
      document.querySelector(".createPage .listWrapper").style.display =
        "block";
      document
        .querySelector(".createPage .typeSelector")
        .classList.add("hidden");
...
async function renderReportInformation(report, type, isList) {
  ...
  const generalInformation = {
    ...
    timeStamp: timeStamp,
    ...(type === "arrest" && report.CourtCaseNumber
  ...
  switch (type) {
    case "incident":
      report.OffenderPedsNames = Array.isArray(report.OffenderPedsNames)
        ? report.OffenderPedsNames
        : [];
      report.WitnessPedsNames = Array.isArray(report.WitnessPedsNames)
        ? report.WitnessPedsNames
        : [];
    ...
    case "arrest":
      ...
      if (type === "arrest") {
        reportInformationEl.dataset.attachedReportIds = JSON.stringify(
          report.AttachedReportIds || [],
        );
        reportInformationEl.dataset.documentedDrugs = String(
          !!report.DocumentedDrugs,
        );
        reportInformationEl.dataset.documentedFirearms = String(
          !!report.DocumentedFirearms,
        );
      }
      if (
        type === "arrest" &&
        (!isList || (report.UseOfForce && report.UseOfForce.Type))
      ) {
        reportBodyMount.appendChild(
        ...
        await getPropertyEvidenceSection(
          {
            ...
            SalesIndicators: report.SalesIndicators || [],
            ManufacturingIndicators: report.ManufacturingIndicators || [],
...
async function getEvidenceSeizedSection(report, isList) {
  ...
  if (items.length > 0) {
    ...
    help.textContent =
      labels.evidenceSeizedHelp ??
      "Attach a Property and Evidence Receipt report below to document seized drugs and firearms. Items from attached PER reports appear here.";
...
function addAttachedReportRow(
  ...
  detachBtn.textContent =
    language.reports?.sections?.arrest?.detach ?? "Detach";
 ...
 */
async function localAttachReports(
  reportInfoEl,
  listWrap,
  report,
  reportIds,
  ...
  const toAdd = reportIds.filter(
    (id) => id && id !== report.Id && !current.includes(id),
  );
  ...
  const doDetach = async (row) => {
    ...
    let ids = [];
    try {
      ids = JSON.parse(reportInfoEl.dataset.attachedReportIds || "[]");
    } catch (_) {}
  ...
  toAdd.forEach((rid) => {
    const sum = Array.isArray(summaries)
      ? summaries.find((s) => s && s.id === rid)
      : null;
    const typeLabel =
      typeLabelByType && typeLabelByType[rid]
        ? typeLabelByType[rid]
        : (sum?.typeLabel ?? "—");
    addAttachedReportRow(
      listWrap,
      reportInfoEl,
      report,
      rid,
...
async function getArrestAttachedReportsSection(report) {
  ...
  section.dataset.title =
    language.reports?.sections?.arrest?.attachedReports ??
    "Attached reports (evidence for court)";
  ...
  const reportInfoEl = document.querySelector(
    ".createPage .listWrapper .reportInformation",
  );
  ...
  help.textContent =
    language.reports?.sections?.arrest?.attachedReportsHelp ??
    "Court reads every attachment. Tight links to the defendant and charges add the most; side reports still help, only at lower weight.";
  section.appendChild(help);
  ...
  const getPersistedArrest = async () => {
    try {
      ...
      return Array.isArray(list)
        ? list.find((r) => r && r.Id === report.Id) || null
        : null;
    } catch (_) {
      return null;
    }
  ...
  attachedIds.forEach((reportId) => {
    ...
    const date = sum?.date
      ? new Date(sum.date + "T00:00:00").toLocaleDateString(undefined, {
          day: "numeric",
          month: "short",
          year: "numeric",
        })
    ...
    const onDetach = async (row) => {
      ...
      if (res === "OK") {
        ...
        if (reportInfoEl && isArrestNotFound(err)) {
          ...
          topWindow.showNotification(
            err?.error || language.reports?.notifications?.saveError || "Error",
            "error",
          );
        }
      }
  ...
  createPerBtn.textContent =
    language.reports?.sections?.arrest?.createPropertyEvidenceReceipt ??
    "Create Property and Evidence Receipt";
  createPerBtn.addEventListener("click", async function () {
    ...
    document
      .querySelectorAll(".createPage .typeSelector .selected")
      .forEach((b) => b.classList.remove("selected"));
    const propBtn = document.querySelector(
      '.createPage .typeSelector [data-type="propertyEvidence"]',
    );
  ...
  importRecentBtn.title =
    language.reports?.sections?.arrest?.importRecentReportsHelp ??
    "Attaches all reports created in the last 60 minutes.";
  importRecentBtn.textContent =
    language.reports?.sections?.arrest?.importRecentReports ??
  ...
  importRecentBtn.addEventListener("click", async function () {
    ...
    try {
      const offenderInput = document.querySelector(
        ".createPage #offenderSectionPedNameInput",
      );
      const offenderName = (
        offenderInput?.value?.trim() ||
        report.OffenderPedName ||
        ""
      ).trim();
      const res = await fetch("/data/recentReports", {
        ...
        body: JSON.stringify({
          withinMinutes: 60,
          pedName: offenderName || undefined,
        }),
      });
      ...
      if (reportInfoEl) {
        try {
          currentAttached = JSON.parse(
            reportInfoEl.dataset.attachedReportIds || "[]",
          );
        } catch (_) {}
      } else {
      ...
      const reportIds = (Array.isArray(recent) ? recent : [])
        .map((r) => r && r.id)
        .filter(
          (id) => id && id !== report.Id && !currentAttached.includes(id),
        );
      if (reportIds.length === 0) {
        topWindow.showNotification(
          language.reports?.sections?.arrest?.importRecentReportsNone ??
            "No new recent reports to import (last 60 min).",
          "info",
        );
      ...
      (Array.isArray(recent) ? recent : []).forEach((r) => {
        if (r && r.id && r.type)
          recentTypeLabels[r.id] =
            r.type === "propertyEvidence"
              ? "Property & Evidence"
              : r.type === "injury"
                ? "Injury"
      ...
      if (!persistedArrest && reportInfoEl) {
        const added = await localAttachReports(
          reportInfoEl,
          listWrap,
          report,
          reportIds,
        ...
        topWindow.showNotification(
          added > 0
            ? (language.reports?.sections?.arrest?.attachReportDraftHint ??
                "Reports will be attached when you save the arrest.") +
                " (" +
                added +
                ")"
            : (language.reports?.sections?.arrest?.importRecentReportsNone ??
                "No new recent reports to import."),
      ...
      if (attachRes.ok && attachData.added > 0) {
        ...
        reportInfoEl &&
        !attachRes.ok &&
        isArrestNotFound(attachData)
      ) {
        const typeLabelByType = {};
        (Array.isArray(recent) ? recent : []).forEach((r) => {
          if (r && r.id && r.type)
            typeLabelByType[r.id] =
              r.type === "propertyEvidence"
                ? "Property & Evidence"
                : r.type === "injury"
                  ? "Injury"
  ...
  attachInput.placeholder =
    language.reports?.sections?.arrest?.attachReportIdPlaceholder ??
    "Report ID (e.g. INC-25-0001, PER-25-0001)";
  attachInput.className = "attachReportIdInput";
  const attachBtn = document.createElement("button");
  attachBtn.type = "button";
  attachBtn.className = "attachReportButton";
  attachBtn.textContent =
    language.reports?.sections?.arrest?.attachReport ?? "Attach report";
  attachBtn.addEventListener("click", async function () {
    ...
    if (!persistedArrest && reportInfoEl) {
      ...
      if (added > 0) {
        ...
        topWindow.showNotification(
          language.reports?.sections?.arrest?.attachReportDraftHint ??
            "Report will be attached when you save the arrest.",
          "success",
        );
      } else {
        topWindow.showNotification(
  ...
  closeArrestBtn.textContent =
    language.reports?.sections?.arrest?.closeArrestSubmit ??
    "Save and close (submit for court)";
  closeArrestBtn.addEventListener("click", async function () {
    ...
    const statusBtn = document.querySelector(
      '.createPage .statusInput button[data-status="0"]',
    );
    if (!statusBtn) return;
    document
      .querySelectorAll(".createPage .statusInput button")
      .forEach((b) => b.classList.remove("selected"));
    statusBtn.classList.add("selected");
    ...
    if (saved) {
      topWindow.showNotification(
        language.reports?.notifications?.closeArrestSuccess ??
          "Arrest closed and submitted for court.",
        "success",
      );
    }
  });
  ...
  if (type === "injury") {
    setTimeout(function () {
      const importBtn = document.querySelector(
        ".createPage .injurySection .importInjuryFromGameBtn",
      );
      if (
        importBtn &&
        document.querySelector("#injurySectionInjuredPartyInput")?.value?.trim()
      ) {
        importBtn.click();
      }
    }, 300);
...
async function saveReport(type, options = {}) {
  ...
  switch (type) {
    ...
    case "arrest":
      ...
      try {
        report.AttachedReportIds = JSON.parse(
          el.dataset.attachedReportIds || "[]",
        );
      } catch {
        report.AttachedReportIds = [];
      }
      ...
      if (uofType) {
        report.UseOfForce = {
          ...
          TypeOther:
            uofType === "Other"
              ? el.querySelector("#useOfForceTypeOtherInput")?.value?.trim() ||
                null
              : null,
          Justification:
            el.querySelector("#useOfForceJustificationInput")?.value?.trim() ||
            null,
          InjuryToSuspect:
            el.querySelector("#useOfForceInjurySuspect")?.checked === true,
          InjuryToOfficer:
            el.querySelector("#useOfForceInjuryOfficer")?.checked === true,
          Witnesses:
            el.querySelector("#useOfForceWitnessesInput")?.value?.trim() ||
            null,
        };
      }
      const drugsEl = el.querySelector("#evidenceSeizedDrugs");
      const firearmsEl = el.querySelector("#evidenceSeizedFirearms");
      report.DocumentedDrugs = drugsEl
        ? drugsEl.checked === true
        : el.dataset.documentedDrugs === "true";
      report.DocumentedFirearms = firearmsEl
        ? firearmsEl.checked === true
        : el.dataset.documentedFirearms === "true";

      response = await postCreateReport("/post/createArrestReport", report);
      break;
    case "impound":
      report.PersonAtFaultName =
        el.querySelector("#impoundSectionPersonAtFaultInput")?.value?.trim() ||
        null;
      report.LicensePlate =
        el.querySelector("#impoundSectionPlateInput")?.value?.trim() || "";
      report.VehicleModel =
        el.querySelector("#impoundSectionModelInput")?.value?.trim() || "";
      report.Owner =
        el.querySelector("#impoundSectionOwnerInput")?.value?.trim() || "";
      report.Vin =
        el.querySelector("#impoundSectionVinInput")?.value?.trim() || "";
      report.ImpoundReason =
        el.querySelector("#impoundSectionReasonInput")?.value?.trim() || "";
      report.TowCompany =
        el.querySelector("#impoundSectionTowInput")?.value?.trim() || "";
      report.ImpoundLot =
        el.querySelector("#impoundSectionLotInput")?.value?.trim() || "";
    ...
    case "trafficIncident": {
      ...
      report.PedestrianNames = collectNames(
        tiLabels.pedestrians || "Pedestrians",
      );
      report.VehiclePlates = collectNames(tiLabels.vehicles || "Vehicles");
      report.VehicleModels = collectNames(
        tiLabels.vehicleModels || "Vehicle Models",
      );
      report.InjuryReported =
        el.querySelector("#trafficIncidentInjuryCheck")?.checked === true;
      report.InjuryDetails =
        el.querySelector("#trafficIncidentInjuryDetailsInput")?.value?.trim() ||
        null;
      report.CollisionType =
        el.querySelector("#trafficIncidentCollisionInput")?.value?.trim() ||
        null;
      response = await postCreateReport(
        "/post/createTrafficIncidentReport",
        report,
      );
      break;
    }
    case "propertyEvidence": {
      report.SubjectPedNames = Array.from(
        el.querySelectorAll(".propertyEvidenceSubjectItem"),
      )
        .map((item) => item.dataset.pedName || "")
        .filter(Boolean);
      report.SubjectPedName =
        report.SubjectPedNames.length > 0 ? report.SubjectPedNames[0] : null;
      report.SeizedDrugs = Array.from(
        el.querySelectorAll(".propertyEvidenceDrugItem"),
      )
        .map((item) => ({
      ...
      report.SeizedFirearmTypes = Array.from(
        el.querySelectorAll(".propertyEvidenceFirearmItem"),
      )
      ...
      response = await postCreateReport(
        "/post/createPropertyEvidenceReceiptReport",
        report,
      );
      break;
    }
    case "injury": {
      ...
      report.InjuredPartyName =
        el.querySelector("#injurySectionInjuredPartyInput")?.value?.trim() ||
        "";
      report.InjuryType =
        el.querySelector("#injurySectionInjuryTypeInput")?.value?.trim() ||
        null;
      report.Severity =
        el.querySelector("#injurySectionSeverityInput")?.value?.trim() || null;
      report.Treatment =
        el.querySelector("#injurySectionTreatmentInput")?.value?.trim() || null;
      report.IncidentContext =
        el.querySelector("#injurySectionContextInput")?.value?.trim() || null;
      report.LinkedReportId =
        el.querySelector("#injurySectionLinkedReportInput")?.value?.trim() ||
        null;
      report.GameInjurySnapshot =
        injurySection?.dataset?.gameInjurySnapshot || null;
  ...
  const reportIdInput = el.querySelector(
    "#generalInformationSectionReportIdInput",
  );
  if (reportIdInput && report.Id) reportIdInput.value = report.Id;
  const attachToArrestId = sessionStorage.getItem(
    "mdtproAttachPropertyEvidenceToArrestId",
  );
  if (type === "propertyEvidence" && attachToArrestId && report.Id) {
    ...
    try {
      const attachRes = await fetch("/post/attachReportToArrest", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          arrestReportId: attachToArrestId,
          reportId: report.Id,
        }),
      });
      ...
      if (attachText === "OK") {
        topWindow.showNotification(
          language.reports?.notifications?.savedAndAttachedToArrest ??
            "Report saved and attached to arrest.",
          "success",
        );
      } else {
        topWindow.showNotification(
...
function showArrestSaveCautionDialog(language) {
  return new Promise((resolve) => {
    ...
    box.style.cssText =
      "background:var(--color-background, #1a1a2e);border:2px solid var(--color-warning, #e6a800);border-radius:8px;padding:1.25rem;max-width:360px;box-shadow:0 4px 20px rgba(0,0,0,0.4);";

    const title = doc.createElement("div");
    title.className = "arrestCautionTitle";
    title.style.cssText =
      "font-weight:bold;color:var(--color-warning, #e6a800);margin-bottom:0.75rem;font-size:1.1rem;";
    title.textContent =
      language.reports?.notifications?.arrestSaveCautionTitle || "Reminder";

    const message = doc.createElement("div");
    message.className = "arrestCautionMessage";
    message.style.cssText =
      "color:var(--color-text, #e0e0e0);line-height:1.45;margin-bottom:1rem;";
    message.textContent =
      language.reports?.notifications?.arrestSaveCautionMessage ||
    ...
    okBtn.style.cssText =
      "background:var(--color-warning, #e6a800);color:#1a1a2e;border:none;padding:0.5rem 1.25rem;border-radius:4px;cursor:pointer;font-weight:bold;";
