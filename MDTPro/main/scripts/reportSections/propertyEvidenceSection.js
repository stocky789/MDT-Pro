...
async function getPropertyEvidenceSection(data = {}, isList = false) {
  ...
  if (!isList) {
    ...
    recentModalBtn.addEventListener("click", () =>
      openPropertyEvidenceRecentIdsModal(section, labels),
    );
  ...
  subjectsLabel.textContent =
    labels.subjectsTitle || "Subjects (persons from whom seized)";
  ...
  if (!isList) {
    ...
    recentIdsLabel.textContent =
      labels.selectFromRecentIds || "Select from Recent IDs";
    ...
    (async function () {
      try {
        ...
        if (recentIds && recentIds.length > 0) {
          ...
          recentIdsList.textContent =
            labels.noRecentIds ||
            "No recent IDs. Collect an ID from a ped to show them here.";
        }
      } catch (e) {
        recentIdsList.textContent =
          labels.recentIdsError || "Could not load Recent IDs.";
      }
    })();
  }
  ...
  function addSubjectToList(parentSection, name) {
    ...
    const existingNames = Array.from(
      parentSection.querySelectorAll(".propertyEvidenceSubjectItem"),
    )
      .map((el) => el.dataset.pedName)
      .filter(Boolean);
    ...
    parentSection
      .querySelector(".propertyEvidenceSubjectsList")
      .appendChild(wrapper);
  }

  const initialSubjects =
    data.SubjectPedNames || (data.SubjectPedName ? [data.SubjectPedName] : []);
  ...
  if (!isList) {
    ...
    addSubjectInput.placeholder =
      labels.addSubjectPlaceholder || "Add subject name";
  ...
  if (!isList && drugTypes.length > 0) {
    ...
    drugsAddLabel.textContent =
      labels.addDrugsHelp ||
      "Select schedule, drug type, and quantity, then click Add";
    ...
    scheduleSelect.setAttribute(
      "aria-label",
      labels.drugSchedule || "Drug schedule",
    );
  ...
  function addDrugToList(parentSection, drugType, quantity) {
    ...
    if (!isList) wrapper.appendChild(delBtn);
    parentSection
      .querySelector(".propertyEvidenceDrugsList")
      .appendChild(wrapper);
  }
  ...
  if (
    initialDrugs.length === 0 &&
    data.SeizedDrugTypes &&
    data.SeizedDrugTypes.length > 0
  ) {
  ...
  if (!isList && firearmTypes.length > 0) {
    ...
    firearmsAddLabel.textContent =
      labels.addFirearmsHelp || "Select firearm type, then click Add";
  ...
  function addFirearmToList(parentSection, firearmType) {
    ...
    if (!isList) wrapper.appendChild(delBtn);
    parentSection
      .querySelector(".propertyEvidenceFirearmsList")
      .appendChild(wrapper);
  }
  ...
  otherLabel.textContent =
    labels.otherContrabandNotes || "Other contraband (optional)";
  const otherInput = document.createElement("textarea");
  otherInput.id = "propertyEvidenceOtherInput";
  otherInput.rows = 3;
  otherInput.placeholder =
    labels.otherContrabandPlaceholder || "Describe other items seized";
  ...
  authBar.textContent =
    labels.authorizationBlurb ||
    "Authorization: I authorize the prosecuting attorney to receive a copy of the related lab report. Signature: ________________  Date: __________";
  ...
  (async function applyBranding() {
    try {
      const res = await fetch(
        "/data/reportBranding?reportType=propertyEvidence",
      );
      ...
      const sealFbEl =
        sealEl && sealEl.querySelector(".report-doc-seal-fallback");
      ...
      if (leftEl)
        leftEl.textContent = (t.leftColumn || "").replace(/\r\n/g, "\n");
      if (sealFbEl)
        sealFbEl.textContent = String(t.centerTitle || "LAB")
          .trim()
          .slice(0, 12);
      if (sealImgEl && sealFbEl) {
        ...
        if (badge && !/\.svg$/i.test(String(badge))) {
          sealImgEl.src =
            "/plugin/DepartmentStyling/image/" + String(badge).trim() + "?v=1";
      ...
      if (rightTitleEl)
        rightTitleEl.textContent = (t.rightTitle || "").replace(/\r\n/g, "\n");
      footer.textContent = t.footer || "";
      if (mainTitleEl && t.propertyEvidenceTitle)
        mainTitleEl.textContent = t.propertyEvidenceTitle;
    } catch (_) {}
  })();
  return shell;
}
